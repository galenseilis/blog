...
Let's look a small example involving two modules `foo` and `bar`.

```
{{< include ./analysis_simple_example/tree.txt >}}
```

In this example we load data into `foo`.
...
![A dependency graph created by the PyDeps Python package](./analysis_simple_example/dependency_graph.png)
...
Let's look at a more involved example now using a design which will accommodate the project becoming large.

```
{{< include ./analysis_spaceflights_pandas/tree.txt >}}
```

![The dependency graph of a de-kedro-ified spaceflight pandas project](./analysis_spaceflights_pandas/dependency_graph.png)
