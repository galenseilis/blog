...
The first way we can use Numba to speed up our code is by compiling a function so that future executions can use the compiled version, removing the need to compile at runtime. Let's take a function that gives us the first *n* Fibonacci numbers, and see how it performs.
...
def fib(n):
    ...
    RETURNS
    x: Array of Fibonacci numbers (numpy.ndarray)
