...
Asynchronous programming is not required for DES, but is it better?
...
Chaos isn't just about high sensitivity to initial conditions. It is also about topological transitivity and density of orbits. IMO, the topological transitivity might be the most important property. Topological mixing is a special case of topological transitivity which may be easier to explain.
...
There is/was a fixed percentage agreement required between a silviculture surveyor and an auditor. I recall a professional forester assuring me that this ensures a reasonable expectation of agreement between surveyor and auditor because the percentages "scale up". They could be right, but I'm skeptical about this. 
...
I expect that the number of natural regen trees within an untreated area (i.e. not planted) follows an inhomogeneous Poisson point process for the number of trees. Recently-planted trees should follow a probability distribution that is close to uniform (but not exactly uniform because it is more like a grid). 

Regardless of the number of trees, the number of trees that a suveyor counts will count has both false positives and false negatives. A first approximation could be a binomial distribution with some probability of successfully counting each tree. But not all trees are equally easy to spot, motivating us towards something like a Poisson binomial distribution. This approach may be more realistic, but if we can get away with a binomial likelihood with a Beta distributed prior on the probability of detection that might be more feasible. Maybe we could even show that some operation like marginalization gives us the binomial distribution from the Poisson binomial.
...
There are, of course, downsides to specialized software. They can sometimes be less configurable out of the box. If what you need for a given project actually is a large multifeatured tool, then writing it yourself might only provide minor benefits.
...
A first approximation could look like this. Suppose a total service completion rate $\mu$, which can be decomposed into the rates for each server:
...
PyMC is a relatively easy win for mixed models. If you have various grouping in the data and you think each population is from the same family of distribution, differing only in the parameters, then PyMC's `coordinates` makes that easy. More complicated structures are also possible.
